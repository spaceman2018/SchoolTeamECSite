...
public class UserInfoDAO {
	...
	// すでに同じログインIDが存在すればそのidを、存在しなければ0を返す
	public int isExistsLoginId(String loginId){
		DBConnector dbConnector= new DBConnector();
		Connection connection= dbConnector.getConnection();
		int id=0;
	...
	public int resetPassword(String loginId, String password){
		DBConnector dbConnector = new DBConnector();
		Connection connection = dbConnector.getConnection();
		String sql="update user_info set password= ?, update_date=now() where user_id= BINARY ?";
		int result=0;
		try{
			PreparedStatement preparedStatement=connection.prepareStatement(sql);
